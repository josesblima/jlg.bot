...
def xpgold_update(user_name, xp, zeni):
    cursor.execute(f'UPDATE levels SET xp = xp + {xp} WHERE user_id = "{user_name}"')
    database.commit()
    cursor.execute(f'UPDATE levels SET zeni = zeni + {zeni} WHERE user_id = "{user_name}"')
    database.commit()
    cursor.execute(f'SELECT * FROM levels WHERE user_id = "{user_name}"')
...
def player_data(user_id):
    cursor.execute(f'SELECT * FROM levels WHERE user_id = "{user_id}"')
    pdata = cursor.fetchone()
    return '‎\n' + 'Player: ' + pdata[0] + '\nxp: ' + str(pdata[1]) + ' lvl: ' + str(pdata[2]) + ' 銭(zeni): ' + str(pdata[3])


  #Check if user is in db, if not, insert it
def check_create_user(user_id):
    cursor.execute(f'SELECT (user_id) FROM levels WHERE user_id = "{user_id}"')
    ...
    if user_exists != True:
        cursor.execute(f'INSERT INTO levels Values("{user_id}", 0, 0, 0)')
        database.commit()
